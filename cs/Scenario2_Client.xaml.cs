...
namespace SDKTemplate
{
    ...
    public sealed partial class Scenario2_Client : Page
    {
        ...
        private StringBuilder csvEcg = new StringBuilder();
        private StringBuilder csvPpg = new StringBuilder();
        ...
        public async void ConnectButton_Click()
        {
            ConnectButton.IsEnabled = false;
        ...
        private void EnableCharacteristicPanels(GattCharacteristicProperties properties)
        {
            ...
            //*****
            if (DisplayHelpers.GetCharacteristicName(selectedCharacteristic) == "RWECGPPG")
            {
                SetVisibility(CharacteristicReadButton, false);
                SetVisibility(CharacteristicWriteButtonInt, false);
                CharacteristicWriteValue.PlaceholderText = "";
                CharacteristicWriteValue.IsEnabled = false;
                
                CharacteristicWriteButton.Content = "Start ECG";
                CharacteristicWriteValue.Text = PrintValues(StartECG(true));
            }
            //*****
        }
        ...
        {
            //*****
            if (CharacteristicWriteButton.Content.Equals("Start ECG"))
            {
                CharacteristicWriteValue.Text = PrintValues(StartECG(true));
                CharacteristicWriteButton.Content = "Stop ECG";

                var writeBuffer = CryptographicBuffer.CreateFromByteArray(StartECG(true));
                var writeSuccessful = await WriteBufferToSelectedCharacteristicAsync(writeBuffer);
            }
            else if (CharacteristicWriteButton.Content.Equals("Stop ECG"))
            {
                CharacteristicWriteValue.Text = PrintValues(StartECG(false));
                CharacteristicWriteButton.Content = "Start ECG";

                var writeBuffer = CryptographicBuffer.CreateFromByteArray(StartECG(false));
                var writeSuccessful = await WriteBufferToSelectedCharacteristicAsync(writeBuffer);
            }
            //*****
            else if (!String.IsNullOrEmpty(CharacteristicWriteValue.Text))
            {
        ...
        private string FormatValueByPresentation(IBuffer buffer, GattPresentationFormat format)
        {
            ...
            else if (data != null)
            {
                ...
                if (DisplayHelpers.GetCharacteristicName(selectedCharacteristic) == "DataECGPPG")
                {
                    try
                    {
                        string currentDataString;
                        if (Convert.ToInt32(data[0]) == 170) //also -86
                        {
                            //return "ECG: " + ParseECGValue(data).ToString();
                            currentDataString = PrintValues(ParseECGValue(data));
                            csvEcg.Append(currentDataString);
                            return "ECG: " + currentDataString;
                        }
                        else if (Convert.ToInt32(data[0]) == 171) //also -85
                        {
                            //return "PPG: " + ParsePPGValue(data).ToString();
                            currentDataString = PrintValues(ParsePPGValue(data));
                            csvPpg.Append(currentDataString);
                            return "PPG: " + currentDataString;
                        }
                        else if (Convert.ToInt32(data[0]) == 131) //also -125
                        {
                            return "QUA_ECG: " + ParseQualityECGValue(data);
                        }
                        else if (Convert.ToInt32(data[0]) == 156) //also -100
                        {
                            string date = DateTime.Now.ToString("dd-MM-yyyy_HH-mm-ss");

                            string ecgPath = string.Format(@"C:\Users\dafne\Desktop\ECGPPG\ECG_{0}.csv", date);
                            string ecgStr = csvEcg.ToString();
                            
                            string ppgPath = string.Format(@"C:\Users\dafne\Desktop\ECGPPG\PPG_{0}.csv", date);
                            string ppgStr = csvPpg.ToString();

                            if (!String.IsNullOrEmpty(ecgStr))
                            {
                                File.WriteAllText(ecgPath, ecgStr);
                                File.WriteAllText(ppgPath, ppgStr);
                            }

                            return "RES_ECG: " + ParseResultECGValue(data);
                        }
                    }
                    catch (ArgumentException)
                    {
                        return "ECG/PPG: (unable to parse)";
                    }
                }
                else if (selectedCharacteristic.Uuid.Equals(GattCharacteristicUuids.HeartRateMeasurement))
                {
                    try
                    {
        ...
        {
            return Convert.ToInt32(Convert.ToDouble(b & Convert.ToByte(255)) * Math.Pow(256.0, Convert.ToDouble(count)));
        }
        private static string ByteToHexString(byte a)
        {
            string s = BitConverter.ToString(new byte[] {a});
            if (s.Length == 1) {
                s = "0$s";
            }
            return s;
        }
        private static ArrayList ParseECGValue(byte[] data)
        {
            ArrayList queueEcg = new ArrayList();
            for (int i=0; i<data.Length/2-1; i++)
            {
                int ecgValue = GetValue(data[i * 2 + 1], 1) + GetValue(data[i * 2 + 2], 0);
                if(ecgValue >= 32768)
                { 
                    ecgValue -= 65536; 
                }
                queueEcg.Add(Convert.ToInt16(-ecgValue));
            }
            return queueEcg;
        }
        private static ArrayList ParsePPGValue(byte[] data)
        {
            ArrayList queuePpg = new ArrayList();
            for (int i = 0; i < data.Length / 2 - 1; i++)
            {
                float ppgValue = (float)(GetValue(data[i * 2 + 1], 1) + GetValue(data[i * 2 + 2], 0));
                if (ppgValue >= 32768)
                {
                    ppgValue -= 65536;
                }
        ...
        private static string ParseQualityECGValue(byte[] data)
        {
            // Byte = [heartValue, hrvValue, quality]
            StringBuilder sb = new StringBuilder();
            int heartValue = GetValue(data[1], 0);
            sb.Append(heartValue.ToString());
            sb.Append(", ");
            float hrvValue = (float)GetValue(data[2], 0);
            sb.Append(hrvValue.ToString());
            sb.Append(", ");
            int quality = GetValue(data[3], 0);
            sb.Append(quality.ToString());
            return sb.ToString();
        }
        private static string ParseResultECGValue(byte[] data)
        {
            // Byte = [resultValue, date, hrv, avBlock, hr, stress, highBp, lowBp, moodValue, breathValue]
            StringBuilder sb = new StringBuilder();
            int resultValue = GetValue(data[1], 0);
            sb.Append(resultValue.ToString());
            sb.Append(", ");
            string date = ByteToHexString(data[10]) + "." + ByteToHexString(data[11]) + "." + ByteToHexString(data[12]) + " " + ByteToHexString(data[13]) + ":" + ByteToHexString(data[14]) + ":" + ByteToHexString(data[15]);
            sb.Append(date);
            sb.Append(", ");
            int hrv = GetValue(data[2], 0);
            sb.Append(hrv.ToString());
            sb.Append(", ");
            int avBlock = GetValue(data[3], 0);
            sb.Append(avBlock.ToString());
            sb.Append(", ");
            int hr = GetValue(data[4], 0);
            sb.Append(hr.ToString());
            sb.Append(", ");
            int stress = GetValue(data[5], 0);
            sb.Append(stress.ToString());
            sb.Append(", ");
            int highBp = GetValue(data[6], 0);
            sb.Append(highBp.ToString());
            sb.Append(", ");
            int lowBp = GetValue(data[7], 0);
            sb.Append(lowBp.ToString());
            sb.Append(", ");
            int moodValue = GetValue(data[8], 0);
            sb.Append(moodValue.ToString());
            sb.Append(", ");
            int breathValue = GetValue(data[9], 0);
            sb.Append(breathValue.ToString());
            return sb.ToString();
        }

        private static byte[] StartECG(bool en)
        {
            ...
            value[value.Length - 1] = (byte)(crc & 255);
